...
class TestLogger(Callback):
    def on_train_begin(self, logs):
        with open('test_file.dat','a') as f:
            pass
        self.testlib = ctypes.CDLL('/home/fei/Documents/Isaac_DRL/penaltyDRL/goalkeeperDRL/examples/blackboard/blackboard.so') #chama a lybrary que contem as funções em c
    ...
    def on_episode_end(self, episode, logs):
        pointer = self.testlib.leitura_int()
        template = 'Episode {0}: reward: {1:.3f}, steps: {2}, goal_scored: {3}, fails: {4}'
        variables = [
            episode + 1,
            logs['episode_reward'],
            logs['nb_steps'],
...
class TrainIntervalLogger(Callback):
    def __init__(self, interval=10000):
        ...
        with open('reward_file.dat','a') as f:
            pass
        self.testlib = ctypes.CDLL('/home/fei/Documents/Isaac_DRL/goalkeeperDRL/goalkeeperDRL/examples/blackboard/blackboard.so') #chama a lybrary que contem as funções em c++
        self.testlib.using_shared_memory()   #usando a função do c++
        self.testlib.leitura_int.restype = ctypes.POINTER(ctypes.c_int) #define o tipo de retorno da função, neste caso a função retorna ponteiro int
